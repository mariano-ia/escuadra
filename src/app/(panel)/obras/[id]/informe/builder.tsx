...
export function ReportBuilder({ obraId, photos }: { obraId: string; photos: Photo[] }) {
  ...
  return (
    <form action={action} className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-start">
      ...
      <div>
        <p className="font-display text-xs tracking-[0.2em] uppercase text-grey-soft mb-3">Fotos</p>
        {photos.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-2">
            {photos.map((p) => (
              <label
                key={p.id}
                className="relative cursor-pointer aspect-square border border-rule overflow-hidden has-[:checked]:ring-2 has-[:checked]:ring-ink"
              >
                <input type="checkbox" name="photo" value={p.id} defaultChecked className="peer sr-only" />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.url} alt="" className="w-full h-full object-cover opacity-50 peer-checked:opacity-100 transition-opacity" />
              </label>
            ))}
          </div>
        )}
